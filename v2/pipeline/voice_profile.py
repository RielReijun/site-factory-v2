"""Voice profile: meet de toon van een prospect uit text.txt.
...
zonder LLM. Concrete signalen:
  ...
  avg_sentence_length   gemiddeld aantal woorden per zin (formaliteit-proxy)
  exclamations_per_1k   uitroeptekens per 1000 woorden (energy-proxy)

Daaruit leiden we af:
  addressing : "je" | "u" | "mixed"
  formality  : "informeel" | "formeel" | "gemengd"

Dit voedt CTA-copy (Bel ons / Bel u / Reserveer online) en hardcoded labels
in render. Geen LLM, geen netwerkcall.
"""
...
# Page-namen die we overslaan in voice-meting (juridisch/footer-ruis verstoort
# de tellingen). Hergebruikt in inventory.py via _SIG_SKIP_PAGES.
...
class VoiceProfile:
    ...
    addressing: str = "unknown"     # "je" | "u" | "mixed" | "unknown"
    formality: str = "gemengd"      # "informeel" | "formeel" | "gemengd"
    sample_words: int = 0
    sample_sentences: int = 0
    method: str = "deterministic"   # "deterministic" | "unavailable"
...
# 'u' is tricky: niet matchen op 'u' midden in woorden. Word-boundary werkt
# omdat \b alleen na/voor non-word-char matcht. 'uw' kan ook stand-alone
# verschijnen als possesief.
_U_RE = re.compile(r"\b(?:u|uw)\b", re.IGNORECASE)
_WORD_RE = re.compile(r"\b[a-zA-ZÀ-ſ'-]+\b")
_SENTENCE_SPLIT_RE = re.compile(r"[.!?]+\s+")

# Pagina-types waarvan body geen indicator is voor de tone-of-voice
# (privacy/cookies/voorwaarden bevatten standaardlanguage van een externe
# generator, niet de echte stem van het bedrijf).
...
def extract_voice_profile(text: str) -> VoiceProfile:
    """Bouw een VoiceProfile uit text.txt. Skip privacy/cookie/voorwaarden-
    pagina's omdat die boilerplate-language bevatten."""
    ...
    # Formaliteits-label: long zinnen + u-vorm = formeel; korte zinnen +
    # je-vorm + uitroepen = informeel.
    if addressing == "u":
        formality = "formeel"
